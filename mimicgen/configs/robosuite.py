...
class Can_Config(MG_Config):
    """
    Corresponds to robosuite PickPlaceCan task and variants (single-can pick-and-place).
    Two subtasks: grasp the can (relative to the can), then place it in its bin. The place
    subtask uses object_ref=None (world frame) since the target bin is fixed -- matching the
    pick_place template's place subtasks.
    """
    NAME = "can"
    TYPE = "robosuite"
