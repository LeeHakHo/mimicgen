...
class MG_Can(RobosuiteInterface):
    """
    Corresponds to robosuite PickPlaceCan (single-object pick-and-place of the can).
    Analogous to MG_Square: one manipulated object (can) + a fixed target (its bin).
    MG_PickPlace can't be reused here because it asserts all four objects are present.
    """
    def get_object_poses(self):
        """
        Gets the pose of each object relevant to MimicGen data generation in the current scene.
        ...
        Returns:
            object_poses (dict): dictionary that maps object name (str) to object pose matrix (4x4 np.array)
        """
        # one relevant object - the can (self.objects holds all four; index by object_to_id)
        can = self.env.objects[self.env.object_to_id["can"]]
        return dict(
            can=self.get_object_pose(obj_name=can.root_body, obj_type="body"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        # first subtask is grasping the can (motion relative to the can)
        can = self.env.objects[self.env.object_to_id["can"]]
        signals["grasp"] = int(self.env._check_grasp(
            gripper=self.env.robots[0].gripper,
            object_geoms=[g for g in can.contact_geoms])
        )

        # final subtask is placing the can into its bin (motion relative to the bin) - but final subtask signal is not needed
        return signals
